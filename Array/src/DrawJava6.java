...
class DrawFrame extends Frame {
	ArrayList<ArrayList<GDraw>> array1 = new ArrayList<ArrayList<GDraw>>();
	ArrayList<GDraw> array2 = new ArrayList<GDraw>();
	...
	public DrawFrame(String title) {
		...
		int x;
		for (int i = 0; i < 3; i++) {
			ArrayList<GDraw> array = new ArrayList<>();
			for (int j = 0; j < 5; j++) {
				//奇数と偶数の時で配置を変える
				if (i != 1)
					x = j * 50 + 50;
				...
				array.add(new Gster(x, i * 50 + 50, Color.yellow));
			}
			array1.add(array);
		}

		array2.add(new Gmoon(400, 100, Color.yellow));

		array2.add(new Gbuilding(150, 350, Color.gray));
		array2.add(new Gbuilding(350, 300, Color.red));
	...
	public void paint(Graphics g) {
		...
		for (ArrayList<GDraw> array: array1)
			for(GDraw d: array)
				d.draw(g);
